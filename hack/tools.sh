...
declare -r BATS_VERSION=${BATS_VERSION:-v1.10.0}
...
declare -r BATS_CORE_REPO="https://github.com/bats-core/bats-core.git"
...
install_bats() {
	[[ -x "$LOCAL_BIN/bats" ]] && {
		ok "bats already installed"
		return 0
	}

	info "installing bats version: $BATS_VERSION"

	local bats_tmp
	bats_tmp="$(mktemp -d)"
	git clone --depth 1 --branch "$BATS_VERSION" \
		"$BATS_CORE_REPO" "$bats_tmp" || {
		fail "failed to clone bats-core"
		rm -rf "$bats_tmp"
		return 1
	}

	"$bats_tmp/install.sh" "$PROJECT_ROOT/tmp" || {
	...
	[[ ! -d "$LOCAL_BIN/bats-support" ]] && {
		git clone --depth 1 \
			"$BATS_SUPPORT_REPO" "$LOCAL_BIN/bats-support" || {
			fail "failed to clone bats-support"
			return 1
		}
	}

	[[ ! -d "$LOCAL_BIN/bats-assert" ]] && {
		git clone --depth 1 \
			"$BATS_ASSERT_REPO" "$LOCAL_BIN/bats-assert" || {
			fail "failed to clone bats-assert"
			return 1
		}
	}

	ok "bats was installed successfully"
}
